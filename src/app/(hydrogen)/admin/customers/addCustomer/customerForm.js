...
const CustomerForm = () => {
  ...
  const handleSave = () => {
    ...
    if (loggedInStatus === true) {
      ...
      const url = `${baseUrl}/api/addAdminCustomer?customerid=${userData.customerid}`;
        ...
        .then((result) => {
          if (result && result.result === 'success') {
            setfName(''),
              ...
              setCountry(''),
              setLoading(false);
            toast.success(<Text as="b">Customer added successfully</Text>);
            router.back();
          } else {
            toast.error(<Text as="b">Error while adding customer</Text>);
          }
          setLoading(false);
        })
        .catch((error) => {
