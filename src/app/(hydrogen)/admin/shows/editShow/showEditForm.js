...
const ShowEditForm = ({ showData }) => {
  ...
  useEffect(() => {
    ...
    setVenueCity(showData?.town);
    setVenueCountry(showData?.county);
    setBuildingNumber(showData?.building_number);
    setVenuePostcode(showData?.postcode);
    setAdultTicketPrice(showData?.price_adult);
    setChildTicketPrice(showData?.price_child);
    setConcessionTicketPrice(showData?.price_concession);
    setTicketCapacity(showData?.ticket_no);
    setSendInvites(showData?.invites);
    setTime(showData?.show_time);
  }, [showData]);
  ...
  const handleSave = () => {
    ...
    if (loggedInStatus === true) {
      ...
      const url = `${baseUrl}/api/editShow?customerid=${userData.customerid}`;
      ...
      formdata.append('showid', showData.id);
      ...
      fetch(url, requestOptions)
        ...
        .then((result) => {
          if (result && result.result === 'success') {
            ...
            toast.error(<Text as="b">Error while updating show</Text>);
          }
          setLoading(false);
        })
        .catch((error) => {
